...
void IR_optimize() {
    ...
    for_vec(IR_function_ptr, i, ir_program_global->functions) {
        ...
        if(debug == 1) VCALL(*constantPropagation, printResult, func);
        ...
        if(debug == 1) VCALL(*copyPropagation, printResult, func);
        ...
        while(true) {
            ...
            if(debug == 1) VCALL(*liveVariableAnalysis, printResult, func);
